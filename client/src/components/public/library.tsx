...
export default function Library() {
  ...
  const currentItem = filteredLibraryItems.find(item => item.id === currentLibraryItemId);

  return (
    <section id="library" className="py-16 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-gray-900 font-serif">Library</h2>
          <div className="mt-2 h-1 w-20 bg-primary mx-auto"></div>
          <p className="mt-4 text-lg text-gray-600">Koleksi foto dan video kegiatan Himpunan</p>
        </div>
                ...
                <div key={item.id} className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow">
                    ...
                    {item.images.length > 1 && (
                      <div>
                        ...
                        <button 
                          onClick={(e) => {
                            e.stopPropagation();
                            setCurrentImageIndex(Math.min(currentImageIndex + 1, item.images.length - 1));
                          }} 
                  ...
                  <div className="p-6">
                    <div className="flex justify-between items-center mb-3">
                      ...
                      <span className="capitalize text-xs px-2 py-1 bg-gray-100 rounded-full">{item.type}</span>
                    </div>
                    ...
                    <Dialog>
                      ...
                      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                        <DialogHeader>
                          <DialogTitle className="text-2xl font-bold font-serif">{item.title}</DialogTitle>
                        </DialogHeader>
                              ...
                              {item.images.length > 1 && (
                                <div className="absolute inset-0 flex items-center justify-between px-4">
                                  ...
                                  <Button
                                    variant="ghost"
                                    size="icon"
                                    onClick={() => setCurrentImageIndex(Math.min(currentImageIndex + 1, item.images.length - 1))}
                        ...
                        <div className="prose max-w-none">{item.fullDescription}</div>
                      </DialogContent>
                    </Dialog>
                  </div>
                </div>
