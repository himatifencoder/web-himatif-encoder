...
export default function Sidebar() {
  ...
  return (
    <aside className={`bg-white border-r border-gray-200 transition-all ${expanded ? 'w-64' : 'w-20'}`}>
      <div className="h-full flex flex-col">
        <div className="p-6 flex items-center justify-between border-b">
          <div className="flex items-center space-x-2">
            {expanded && <span className="font-bold text-xl text-primary">HMTI</span>}
            {!expanded && <span className="font-bold text-xl text-primary">H</span>}
          </div>
          <Button
            variant="ghost"
            size="icon"
        ...
        <div className="p-4 border-t">
          <div className={`flex ${expanded ? 'items-center' : 'flex-col items-center'} space-x-3`}>
            <Avatar>
