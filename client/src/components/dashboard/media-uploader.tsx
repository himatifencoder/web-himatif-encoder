...
import { Loader2, X, Upload, Plus, Image as ImageIcon, Video } from "lucide-react";
...
export default function MediaUploader({ item, onSave, onCancel }: MediaUploaderProps) {
  const { toast } = useToast();
  const [title, setTitle] = useState(item?.title || "");
  const [description, setDescription] = useState(item?.description || "");
  const [fullDescription, setFullDescription] = useState(item?.fullDescription || "");
  const [mediaType, setMediaType] = useState<"photo" | "video">(item?.type || "photo");
  ...
  const saveMediaMutation = useMutation({
    ...
    onError: (error) => {
      toast({
        title: "Error",
        description: "Failed to save the media item. Please try again.",
        variant: "destructive",
      });
    ...
    <div className="space-y-6">
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <RadioGroup value={mediaType} onValueChange={(value) => setMediaType(value as "photo" | "video")} className="flex space-x-4">
