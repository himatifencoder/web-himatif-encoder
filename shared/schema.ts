...
export const userInsertSchema = createInsertSchema(users, {
  ...
  role: (schema) => schema.refine((val: string) => ['owner', 'admin', 'chair', 'vice_chair', 'division_head'].includes(val), {
...
export const articleUpdateSchema = createInsertSchema(articles, {
  ...
  excerpt: (schema) => schema.min(10, "Excerpt must be at least 10 characters").optional(),
  content: (schema) => schema.min(20, "Content must be at least 20 characters").optional(),
...
export const libraryInsertSchema = createInsertSchema(library, {
  ...
  fullDescription: (schema) => schema.min(20, "Full description must be at least 20 characters"),
  type: (schema) => schema.refine((val: string) => ['photo', 'video'].includes(val), {
...
export const libraryUpdateSchema = createInsertSchema(library, {
  ...
  fullDescription: (schema) => schema.min(20, "Full description must be at least 20 characters").optional(),
  type: (schema) => schema.refine((val: string) => ['photo', 'video'].includes(val), {
    message: "Type must be either photo or video"
  }).optional()
});
...
export const organizationInsertSchema = createInsertSchema(organization, {
  ...
  period: (schema) => schema.regex(/^\d{4}-\d{4}$/, "Period must be in format YYYY-YYYY")
});

export const organizationUpdateSchema = createInsertSchema(organization, {
  name: (schema) => schema.min(3, "Name must be at least 3 characters").optional(),
  position: (schema) => schema.min(3, "Position must be at least 3 characters").optional(),
  period: (schema) => schema.regex(/^\d{4}-\d{4}$/, "Period must be in format YYYY-YYYY").optional()
});

export const settingsSchema = createInsertSchema(settings);

export const settingsInsertSchema = createInsertSchema(settings, {
  siteName: (schema) => schema.min(3, "Site name must be at least 3 characters"),
  siteTagline: (schema) => schema.min(3, "Site tagline must be at least 3 characters"),
  contactEmail: (schema) => schema.email("Invalid email format")
});

export const settingsUpdateSchema = createInsertSchema(settings, {
  siteName: (schema) => schema.min(3, "Site name must be at least 3 characters").optional(),
  siteTagline: (schema) => schema.min(3, "Site tagline must be at least 3 characters").optional(),
